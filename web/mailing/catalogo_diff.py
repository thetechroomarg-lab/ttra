...
Los equipos usados/CPO nunca entran en la campaña — ni como "nuevos" ni
como relleno — sin excepción."""
import random
...
def detectar_nuevos(productos_actuales, snapshot_anterior):
    ...
    return [
        p for p in productos_actuales
        if p.get("nombre") not in nombres_anteriores and not es_usado(p)
    ]
...
def seleccionar_para_campania(nuevos, catalogo_completo, cantidad=10):
    ...
    - Si hay menos, se completa al azar con el resto del catálogo (no nuevos,
      sin usados) hasta llegar a `cantidad`, o hasta agotar el catálogo si no
      alcanza."""
    nuevos = [p for p in nuevos if not es_usado(p)]
    if len(nuevos) >= cantidad:
        return random.sample(nuevos, cantidad)

    nombres_nuevos = {p.get("nombre") for p in nuevos}
    resto = [
        p for p in catalogo_completo
        if p.get("nombre") not in nombres_nuevos and not es_usado(p)
    ]

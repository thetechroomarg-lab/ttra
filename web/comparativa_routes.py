...
def install(app, catalog_loader, data_path, client_factory):
    ...
    def comparison_sheet(payload: SheetRequest, request: Request):
        ...
        try:
            ...
            # Tope bajo a propósito: cada búsqueda tiene costo real (~4 centavos c/u),
            # y cada producto ya queda cacheado 30 días (SpecStore), así que esto solo
            # limita productos *nuevos* sin comparar todavía. 3 por usuario (IP) por día.
            if not limiter.reserve(('comparison', peer), 3, 86400) or not app.state.comparison_global_budget.reserve(('comparison', 'global'), 20, 86400):

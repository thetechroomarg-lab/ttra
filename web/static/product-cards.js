...
(() => {
  ...
  function tick(now){
    frame=0;if(!engine||!active.size||!allowed()||reduced.matches){last=0;return;}
    if(!last)last=now;
  ...
  function sync(){stop();entries.forEach(({card})=>card.classList.toggle('ttra-scene-active',active.has(card)&&allowed()&&!reduced.matches));draw();if(engine&&active.size&&allowed()&&!reduced.matches)frame=requestAnimationFrame(tick);}
  ...
  entries.forEach(({card})=>visibility.observe(card));

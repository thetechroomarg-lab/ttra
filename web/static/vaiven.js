...
// Reuse Vaivén's actual face, with a seated, forward-facing body for his album.
const mascot=document.getElementById('vaiven-mascot');
const source=document.createElement('template');source.innerHTML=catArtwork;
...
mascot.innerHTML=`<svg viewBox="0 0 120 136" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <g fill="#fffdf8" stroke="#d8cfc4" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
    <path d="M38 117Q13 128 15 100" fill="none" stroke="#c4a07c" stroke-width="10"/>
    <ellipse cx="60" cy="102" rx="28" ry="30"/>
    <ellipse cx="60" cy="99" rx="15" ry="23" fill="#f1eee7" stroke="none"/>
    ...
    <path d="M51 125V129M55 126V130M67 126V130M71 125V129" stroke="#d8cfc4" stroke-width="1"/>
    <g class="vaiven-face" transform="translate(-18 -3)"></g>
    <path class="vaiven-scratch-paw" d="M82 117Q101 103 91 73" fill="none" stroke="#fffdf8" stroke-width="10"/>
  </g>
</svg>`;
mascot.querySelector('.vaiven-face').append(head);

// Content remains visible if animation APIs are unavailable.
if ('IntersectionObserver' in window && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
  document.documentElement.classList.add('vaiven-ready');
  ...
  document.querySelectorAll('[data-vaiven-reveal]').forEach(el=>observer.observe(el));
}

// This exception belongs only to the album's return button and is consumed once.
const returnHome=document.querySelector('.vaiven-back');
...
returnHome?.addEventListener('click',async event=>{
  ...
  try { sessionStorage.setItem('ttra_vaiven_return_once',JSON.stringify({guest,at:Date.now()})); } catch {}
